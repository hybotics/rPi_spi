...
static PyObject* openSPI(PyObject *self, PyObject *args, PyObject *kwargs) {
	...
	*/
	...
	/*
		It's not clearly documented, but it seems that PyArg_ParseTupleAndKeywords basically only modifies the values passed to it
			if the keyword pertaining to that value is passed to the function. As such, the defaults specified by the variable
			definition are used unless you pass a kwd argument.

		Note that there isn't any proper bounds-checking, so if you pass a value that exceeds the variable size, it's just truncated
			before being stuffed into the avasilable space. For example, passing a bits-per-word of 500 gets truncated to 244.

		Unfortunately, the PyArg_ParseTupleAndKeywords function only seems to support ints of 32 bits.
	*/
	...
	*/
	...
	/*
		...
		Note that the returned values may not be completely real. It seems that, at least for the speed value, the hardware
			only has several possible settings (250000, 500000, 1000000, etc...) Strangely enough, the ioctl for setting
			the speed *returns the speed you specify*. However, the hardware seems to default to the closest avalable value
			*below* the specified rate. (i.e. you will never get a speed faster then you specify), but you may get a slower value.

		It would probably be a good idea to bin-down the passed arguement to the available values, and return that.
	*/
	PyObject* retDict;
	retDict = PyDict_New();
...
static PyObject* transferSPI (PyObject* self, PyObject* arg) {
...
static PyMethodDef SpiMethods[] = {
	...
	{"transferSPI", transferSPI, METH_VARARGS, "Transfer data."},
...
PyMODINIT_FUNC
initrPi_spi (void) {
	(void) Py_InitModule("rPi_spi", SpiMethods);
}
